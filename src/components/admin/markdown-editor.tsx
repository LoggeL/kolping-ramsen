...
export function MarkdownEditor({
  ...
  const [commandList, setCommandList] = useState<MdCommand[] | undefined>(
    undefined,
  );
  ...
  const mediaCommand = useMemo<MdCommand>(
    () => ({
      name: "insert-media",
      keyCommand: "insert-media",
      buttonProps: {
        "aria-label": "Bild aus Mediathek einfügen",
  ...
  useEffect(() => {
    ...
    import("@uiw/react-md-editor").then((mod) => {
      if (cancelled) return;
      const c = (mod as unknown as { commands: Record<string, MdCommand> })
        .commands;
      const list: MdCommand[] = [
        c.bold,
        c.italic,
        c.strikethrough,
        c.hr,
        ...
        c.table,
        c.divider,
        c.unorderedListCommand,
        c.orderedListCommand,
        c.checkedListCommand,
      ].filter(Boolean);
      setCommandList(list);
    });
    return () => {
